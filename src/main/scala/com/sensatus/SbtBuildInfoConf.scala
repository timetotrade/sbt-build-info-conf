...
object SbtBuildInfoConf extends AutoPlugin {
  ...
  lazy val GitBranch: Try[String] = {
    Try {
      val builder    = new FileRepositoryBuilder
      val repository = builder.readEnvironment.findGitDir.build()
      Option(repository.getBranch)
        .filter(_ != GitCommit).orElse {
          val head = repository.exactRef(org.eclipse.jgit.lib.Constants.HEAD)
          repository.getAllRefs
            .values().iterator().asScala.find { ref ⇒
              !"HEAD".equals(ref.getName) && ref.getObjectId.equals(head.getObjectId)
  ...
  override lazy val projectSettings = Seq(
    ...
    mappings in (Compile, packageBin) := (mappings in (Compile, packageBin)).value
      .filter(!_._1.getName.contains("reference.conf")),
    /*
     * Add our newly generated conf file back and rename it
     */
    mappings in (Compile, packageBin) += ((resourceManaged in Compile).value / "gen-reference.conf") → "reference.conf"
  )
  ...
  def generateResource(
    ...
    val df = GitDirtyFiles
      .map(
        t ⇒
          conf.withValue(
            org + ".buildinfo." + mod + ".git.dirtyFiles",
            ConfigValueFactory.fromMap(t.asJava)
        )
      ).getOrElse {
